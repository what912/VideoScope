...
Production: `https://what912.github.io/VideoScope/`

Source: `https://github.com/what912/VideoScope`

Status: **LIVE OVER HTTPS**

This checklist separates verified local/remote evidence from manual gates that
remain open. A checked item means the command, workflow, or inspection was
actually completed; unchecked Supabase, accessibility, export, browser, and
network-panel gates must not be inferred from the successful Pages deployment.
...
| Check | Result |
| --- | --- |
| GitHub CI run `30665501160` | PASS; React job, Ubuntu/Windows Python 3.11/3.12, distribution build/audit, and wheel smoke test succeeded |
| GitHub Pages run `30665501278` | PASS; generated media, lint, strict TypeScript, 512 frontend tests, build, media verification, artifact upload, and deployment succeeded |
| `npm run lint` / `npm run typecheck` / `npm test` / `npm run build` | PASS locally; 512 frontend tests passed before this documentation-only follow-up |
| `npm run media:prepare` | PASS locally and remotely; generated 15 project-authored media outputs with no remote input |
| `npm run media:verify` | PASS in GitHub Pages; local rerun is blocked because this shell's `PATH` does not expose ffprobe |
| `npm run media:review` | PASS locally; generated the ignored 3x3 `runs/media-review/contact-sheet.webp` from all nine manifest posters |
| `python scripts/validate.py` | PASS locally; 218 tests passed and 14 were skipped |

The initial public deployment baseline was root commit
`651b0643191a12f4474ae62db5d3df7d1e5e65fa`, with tree
`551a532cf7b423bbffdbe09c1010dc0e6e992b2e`, zero parents, and 438 paths.
These identifiers describe source identity, not detector accuracy.
...
Production acceptance was performed signed out over HTTPS. Home, Workspace
(including direct load and reload), Compare, `/report/demo`, Privacy, Docs, and
Auth loaded. The interface switched from Simplified Chinese to English while
the literal `what912` creator mark remained unchanged. The bundled sample ran
all four CPU detectors and produced report
`browser-717e810e9a5cabc4052a` with zero intervals, detector executions,
timeline/evidence surfaces, and reload recovery. The inspected page console had
no warnings or errors.

Raw deep URLs receive GitHub Pages HTTP 404 before `public/404.html` performs
same-origin client restoration. Browser navigation and reload recovery passed,
but consumers that treat the initial status as terminal may not recover.
...
- [x] Public repository `what912/VideoScope` exists and contains the reviewed
  root commit.
- [x] User explicitly authorized publication to a public `what912/VideoScope`
  repository and GitHub Pages.
- [x] GitHub Pages is enabled for the Actions deployment source and HTTPS is
  enforced.
- [x] Pages workflow `30665501278` completes successfully.
- [x] Signed-out production checks pass for:
...
GitHub source publication and Pages deployment have been performed. No GitHub
Release, PyPI upload, or Supabase production mutation has been performed.
Supabase auth/share remains visibly unavailable and must stay disabled until
every production gate above is complete.
