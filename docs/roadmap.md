...
1. 精确来源、配对码、短期会话和内存密钥保险库；
2. OpenAI-compatible BYOK 内容智能 Provider 与离线 Fake 测试；
3. 公开站 Connector 检测、配对、能力显示和完整模式跳转；
4. 加密本地设备账户、备份导入导出和可选云登录分层；
5. A/B/C/D 与高级 AI 的公开站到本机完整链路验收；
6. GitHub Pages 发布、安全审计和安装引导。

退出条件不是“网页看起来可用”，而是真实本地处理、密钥不落盘、
Provider 故障隔离、浏览器离线退化以及双语说明均通过验证。
