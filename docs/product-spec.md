...
用户明确批准的 v0.8 方向在不改变 v0.1 CPU 契约的前提下增加：GitHub
Pages 作为统一公开入口；浏览器 Quick Scan 无需安装或登录；完整 A/B/C/D、
FFmpeg、长视频和高级 AI 通过回环 Local Connector；AI 使用用户自带
API Key 并由用户与供应商直接结算；默认身份为加密的本地设备账户。

VideoScope 不建立项目方承担费用的云视频存储、分析队列或模型代理。
远程 AI 仍是显式授权、可失败的增强层，不得替换 CPU 事实层或生成未经
校准总分。详细边界见 `docs/zero-cost-access.md`。
