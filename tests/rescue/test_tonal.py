...
def test_probe_audio_retries_once_after_semantically_incomplete_payload(
    ...
    def runner(arguments: tuple[str, ...]) -> CommandResult:
        nonlocal attempts
        attempts += 1
        payload = first_payload if attempts == 1 else expected
...
def test_probe_audio_retries_once_after_duplicate_required_stream(
    first_payload: dict[str, object],
) -> None:
    expected = _renderer_probe()
    attempts = 0
...
def test_probe_audio_fails_closed_after_two_semantically_incomplete_payloads() -> None:
    attempts = 0
    incomplete = {
        "format": {"duration": "3.0"},
        "streams": [
    ...
    def runner(arguments: tuple[str, ...]) -> CommandResult:
        nonlocal attempts
        attempts += 1
        return CommandResult(0, "", json.dumps(incomplete))
...
def test_probe_audio_does_not_retry_complete_unsupported_audio_layout() -> None:
    unsupported = _renderer_probe()
    unsupported["streams"] = [
        {"codec_type": "video", "codec_name": "h264"},
        {
            "codec_type": "audio",
    ...
    def runner(arguments: tuple[str, ...]) -> CommandResult:
        nonlocal attempts
        attempts += 1
        return CommandResult(0, "", json.dumps(unsupported))
    ...
    assert attempts == 1
    with pytest.raises(RescueMediaError) as caught:
        tonal_module._audio_properties(payload, TonalInterferenceConfig())
    assert caught.value.internal_message == "tonal audio layout is unsupported"
